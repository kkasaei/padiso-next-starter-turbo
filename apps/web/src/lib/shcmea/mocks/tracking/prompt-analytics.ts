...
import type {
  ...
  InsightItem,
  RecommendationItem,
} from '@/types/dtos/prompt-analytics-dto'
...
export function aggregateProviderMentions(
  ...
  return [
    { name: 'ChatGPT', value: total > 0 ? Math.round((totals.ChatGPT / total) * 100) : 45, color: '#10b981' },
    { name: 'Perplexity', value: total > 0 ? Math.round((totals.Perplexity / total) * 100) : 35, color: '#8b5cf6' },
    { name: 'Gemini', value: total > 0 ? Math.round((totals.Gemini / total) * 100) : 20, color: '#3b82f6' },
  ]
}
