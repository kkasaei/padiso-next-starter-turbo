...
import { ACTIVITY_TYPE_LABELS, CONTENT_STATUS_CONFIG } from '@/lib/common/constants'
...
// Fetches aggregated analytics from ClickHouse via getProjectAnalytics
// ============================================================
...
function MetricsGrid({ projectId, hasData = true, dashboardStats }: MetricsGridProps) {
...
// Fetches real content drafts from the database
// ============================================================
function ContentTab({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    setIsLoading(true)
  }, [projectId, statusFilter, searchQuery, currentPage, pageSize])
...
// Fetches real activities from the database
// ============================================================
function ActivitiesTab({ projectId }: { projectId: string }) {
  const [currentPage, setCurrentPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)
  ...
  // Fetch activities on mount and when pagination changes
  useEffect(() => {
    setIsLoading(true)
  ...
  const formatMetadata = (activity: any) => {
...
// ============================================================
function OpportunitiesChart({ projectId }: { projectId: string }) {
  const { dateRange, presetId } = useAnalyticsDateRange()
...
function PageContent() {
  ...
  // Determine if project has data based on actual tracked prompts
...
// ============================================================
// MAIN PAGE COMPONENT
// Wraps content with AnalyticsDateRangeProvider
// ============================================================
