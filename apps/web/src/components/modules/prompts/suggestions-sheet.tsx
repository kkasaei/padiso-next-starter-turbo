...
import { listSuggestionsAction } from '@/actions/prompt-suggestion/list-suggestions'
import { generateSuggestionsAction } from '@/actions/prompt-suggestion/generate-suggestions'
import { useSuggestionAction } from '@/actions/prompt-suggestion/use-suggestion'
import { getSuggestionCountAction } from '@/actions/prompt-suggestion/get-suggestion-count'
...
// Type for the suggestion from the server
interface SuggestionFromServer {
  id: string
  title: string
  prompt: string
  category: string | null
...
export function SuggestionsSheet({
  ...
  const { execute: executeListSuggestions } = useAction(listSuggestionsAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
        setSuggestions(data.suggestions.map((s: SuggestionFromServer) => ({
          id: s.id,
          title: s.title,
          prompt: s.prompt,
          category: s.category,
  ...
  const { execute: executeGenerateSuggestions } = useAction(generateSuggestionsAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
        toast.success('Generating suggestions...', {
          description: 'This may take a few seconds',
        })
        // Poll for new suggestions after a delay
        setTimeout(() => {
          loadSuggestions()
        }, 5000)
      } else {
  ...
  const { execute: executeUseSuggestion } = useAction(useSuggestionAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
        // Update local state to mark as used
        setSuggestions((prev) =>
          prev.map((s) =>
            s.id === usingSuggestionId ? { ...s, isUsed: true } : s
          )
        )

        // Find the suggestion and call the callback
        const suggestion = suggestions.find((s) => s.id === usingSuggestionId)
        if (suggestion) {
          onSelectSuggestion({
            id: suggestion.id,
        ...
        toast.error(data?.message || 'Failed to use suggestion')
      }
  ...
  const { execute: executeGetSuggestionCount } = useAction(getSuggestionCountAction, {
    onSuccess: ({ data }) => {
      if (data?.success) {
  ...
  const loadSuggestions = useCallback(() => {
    if (!projectId) return
    setIsLoading(true)
    executeListSuggestions({ projectId, includeUsed: false })
    executeGetSuggestionCount({ projectId })
  }, [projectId, executeListSuggestions, executeGetSuggestionCount])
  ...
  const handleGenerate = useCallback(() => {
    ...
    executeGenerateSuggestions({ projectId, count: 10 })
  }, [canGenerate, projectId, executeGenerateSuggestions])
  ...
  }, [isOpen, projectId, loadSuggestions])
  ...
  }, [isOpen, isLoading, suggestions.length, canGenerate, isGenerating, handleGenerate])
  ...
  const handleSelectSuggestion = (suggestion: SuggestionItem) => {
    ...
    executeUseSuggestion({ suggestionId: suggestion.id })
  }
