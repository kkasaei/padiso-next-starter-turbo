...
// Clerk role constants
const CLERK_ROLES = ['org:member', 'org:admin'] as const;
...
export const profileOnboardingSchema = z.object({
  action: z.nativeEnum(FileUploadAction, {
    ...
    invalid_type_error: 'Action must be a valid value'
  }),
  image: z
    .string({
  ...
  invitations: z
    .array(
      z.object({
        ...
        role: z.enum(CLERK_ROLES, {
          required_error: 'Role is required',
          invalid_type_error: 'Role must be a valid Clerk role'
        })
      })
    )
